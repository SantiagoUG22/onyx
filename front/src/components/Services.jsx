...
const services = [
  ...
  },
  {
    ...
    icon: Sparkles,
    color: 'from-onyx-accent to-onyx-gold',
    glow: 'shadow-onyx-gold/20',
    tags: ['Impacto', 'Seguro']
  },
  {
    ...
    icon: PartyPopper,
    color: 'from-onyx-light to-onyx-gold',
    glow: 'shadow-onyx-gold/20',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <h2 className="text-5xl md:text-7xl font-black mb-8 tracking-tighter">
            NUESTROS <span className="text-onyx-gold">SERVICIOS</span>
          </h2>
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-10">
                {service.tags.map((tag, idx) => (
                  <span key={idx} className="text-[9px] md:text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-full bg-white/5 text-gray-400 border border-white/10 group-hover:border-onyx-gold/30 group-hover:text-gray-300 transition-all flex-shrink-0">
                    {tag}
                  </span>
                ))}
              </div>

              <h3 className="text-2xl font-bold mb-6 text-white group-hover:text-onyx-gold transition-colors duration-300">
                {service.title}
              </h3>
              
              <p className="text-gray-500 font-light leading-relaxed mb-12 text-sm group-hover:text-gray-400 transition-colors">
                {service.description}
              </p>

              <div className="space-y-4 pt-8 border-t border-white/5">
                <div className="flex items-center gap-3 text-xs text-gray-500 group-hover:text-gray-300 transition-colors">
                  <div className="w-5 h-5 rounded-full bg-onyx-gold/10 flex items-center justify-center">
                    <CheckCircle2 size={12} className="text-onyx-gold" />
                  </div>
                  Montaje Profesional
                </div>
                <div className="flex items-center gap-3 text-xs text-gray-500 group-hover:text-gray-300 transition-colors">
